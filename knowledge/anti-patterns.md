...
Next.js dev mode inside a Docker container causes infinite recompile loops. A frontend dashboard hit 38,000% CPU. Always use production builds (multi-stage Dockerfile: node build -> nginx serve) for containerized frontends.
Source: vap dashboard incident. See memory: project_ulimit_fix.md.

### Treating gastown/beads/mesh as user projects (2026-03-31)
These are TOOLS, not products. Don't create beads in the gastown rig for town infrastructure work. Town-level work uses de- prefix beads. User's actual products: project_gamma, project_delta, project_beta, project_alpha, etc.
...
Source: your-github-account suspension.
