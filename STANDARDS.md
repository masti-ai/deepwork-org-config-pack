...
| **scope** | Bead ID (e.g. `pa-b65l`) or area (e.g. `dashboard`, `api`, `mobile`) |
...
**Examples:**
```
feat(pa-b65l): add confidence score to API response
...
docs(pa-ci): update deploy pipelines for live AWS infra
```
...
**Examples:**
```
gt/polecat-nitro/pa-b65l-add-confidence
gt/user/pd-dws-a59-vercel-deploy
gt/crew-mel/pb-dd1-whatsapp-gateway
```
...
**Validation tooling:** Use `scripts/wl-validate.py` or `scripts/wl-post.sh` to validate items before posting. Both enforce required fields, valid types/priorities, description structure, and block private info leaks. Template at `templates/wl-template.yaml`.
...
| Wasteland format | Witness patrol + wl-validate.py | LLM writes rich descriptions; validator blocks private info |
